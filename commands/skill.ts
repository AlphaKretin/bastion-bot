...
const func = async (msg: Message, mobile: boolean): Promise<Message> => {
	...
	if (!skill) {
		return await msg.channel.createMessage(
			"Sorry, I could not find a Skill with a name like `" +
				content +
				"`! For a Speed Duel Skill, try a normal card search."
		);
	}
	if (mobile) {
		return await msg.channel.createMessage(
...
const desc =
	"Searches for a Duel Links skill by name and returns details about it. For a Speed Duel skill, try a normal card search.";
